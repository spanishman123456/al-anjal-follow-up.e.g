...
def _enrich_student_single_quarter(
    ...
    if quarter == 2:
        ...
        student["quarter2_practical"] = effective.get("quarter2_practical")
        ...
        student["focus_final_practical"] = effective.get("quarter2_practical")
        student["focus_final_theory"] = effective.get("quarter2_theory")
        ...
        student["quarter1_practical"] = effective.get("quarter1_practical")
        ...
        student["focus_final_practical"] = effective.get("quarter1_practical")
        student["focus_final_theory"] = effective.get("quarter1_theory")
...
def _focus_component_snapshot(student: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "focus_final_practical": student.get("focus_final_practical"),
        "focus_final_theory": student.get("focus_final_theory"),
    }
...
def create_component_breakdown_bar_chart(student: Dict[str, Any], quarter: int) -> io.BytesIO:
    ...
    bars = ax.bar(x, values, color=colors_list, edgecolor="white", linewidth=0.8, width=0.65)
    ax.set_ylim(0, max(ymax * 1.18, 5))
    for i, bar in enumerate(bars):
        h = bar.get_height()
        ax.text(
            bar.get_x() + bar.get_width() / 2,
...
def create_reports_enrollment_bar_chart(class_breakdown: List[Dict[str, Any]]) -> io.BytesIO:
    ...
    bars = ax.bar(x, counts, color=BOARD_ANALYTICS["bar"], edgecolor="white", linewidth=0.8, width=0.65)
    ax.set_ylim(0, y_top)
    for i, bar in enumerate(bars):
        h = bar.get_height()
        ax.text(
...
def create_reports_enrollment_area_chart(class_breakdown: List[Dict[str, Any]]) -> io.BytesIO:
    """Enrollment profile across classes — same series as Reports `ClassScoreArea` with enrollment bars."""
    if not class_breakdown:
        return _analytics_empty_chart("No class enrollment data")
...
def generate_reports_dashboard_pdf(
    report: Dict[str, Any],
    scope: Any,
    insights: Optional[Dict[str, str]] = None,
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            ...
            if selected_student:
                selected_df = pd.DataFrame([
                    {
                        "Student": selected_student.get("full_name"),
                        "Class": selected_student.get("class_name"),
                        "Quarter total (50)": _focus_total(selected_student),
                        "Performance": selected_student.get("performance_level_q2") if rq == 2 else selected_student.get("performance_level_q1"),
                        **{label: selected_student.get(key) for label, key in component_defs},
                    }
                ])
                selected_df.to_excel(writer, sheet_name="Selected Student", index=False)
        buffer.seek(0)
    ...
    top_df = pd.DataFrame([
        {
            ...
            **{label: student.get(key) for label, key in component_defs},
        }
...
async def get_admin_notification_recipients() -> List[str]:
    admins = await db.users.find({"role_name": "Admin", "active": True}, {"_id": 0, "email": 1}).to_list(50)
    recipients: List[str] = []
    for admin in admins:
        email = (admin.get("email") or "").strip()
        if email and email not in recipients:
            recipients.append(email)
    fallback = (os.environ.get("ADMIN_EMAIL") or "").strip()
    if fallback and fallback not in recipients:
        recipients.append(fallback)
    return recipients


def send_gmail_request_email(recipients: List[str], pending_user: Dict[str, Any]):
    api_key = os.environ.get("SENDGRID_API_KEY")
    if not api_key:
        raise RuntimeError("SENDGRID_API_KEY not configured")
    sender = get_sender_identity()
    subject = f"Gmail login approval required: {pending_user.get('name') or pending_user.get('email') or 'New user'}"
    html_content = (
        "<p>A new Gmail sign-in is waiting for admin approval.</p>"
        f"<p><strong>Name:</strong> {escape(str(pending_user.get('name') or ''))}</p>"
        f"<p><strong>Email:</strong> {escape(str(pending_user.get('email') or ''))}</p>"
        f"<p><strong>Requested at:</strong> {escape(str(pending_user.get('gmail_requested_at') or pending_user.get('created_at') or ''))}</p>"
        "<p>Open the website Settings page to approve or reject this Gmail account.</p>"
    )
    message = Mail(
        from_email=sender,
...
class UserBase(BaseModel):
    ...
    gmail_approval_status: Optional[str] = None
    gmail_requested_at: Optional[str] = None
    gmail_approved_at: Optional[str] = None
    gmail_approved_by: Optional[str] = None
...
class GmailPendingApprovalAction(BaseModel):
...
class RewardBadgeRequest(BaseModel):
    student_id: str
...
@api_router.post("/gmail-pending-users/{user_id}/approve", response_model=UserRecord)
async def approve_gmail_pending_user(
    user_id: str,
    payload: GmailPendingApprovalAction,
    ...
    if not result:
        raise HTTPException(status_code=404, detail="User not found")
    message = (
        f"Approved Gmail sign-in for {result.get('name') or ''} ({result.get('email') or ''})."
        + (f" Note: {payload.note.strip()}" if payload.note and payload.note.strip() else "")
    )
    await log_notification("gmail_login_approved", message, result.get("email") or "", "approved")
    await log_audit("Gmail account approved", result)
    return result


@api_router.post("/gmail-pending-users/{user_id}/reject", response_model=UserRecord)
async def reject_gmail_pending_user(
    user_id: str,
    payload: GmailPendingApprovalAction,
    current_user: Dict[str, Any] = Depends(require_admin),
):
    ...
    if not result:
        raise HTTPException(status_code=404, detail="User not found")
    message = (
        f"Rejected Gmail sign-in for {result.get('name') or ''} ({result.get('email') or ''})."
        + (f" Note: {payload.note.strip()}" if payload.note and payload.note.strip() else "")
    )
    await log_notification("gmail_login_rejected", message, result.get("email") or "", "rejected")
    await log_audit("Gmail account rejected", result)
    return result
...
@api_router.post("/rewards/award-badge", response_model=RewardBadgeResponse)
async def reward_award_badge(payload: RewardBadgeRequest):
    normalized_perf = normalize_reward_performance(payload.performance)
    if normalized_perf not in {"advanced", "on_level"}:
        raise HTTPException(
            status_code=400,
            detail="Badge award is allowed only for On Level / Advanced students.",
        )
    event = {
        "id": str(uuid.uuid4()),
...
async def reward_remove_badge(payload: RewardBadgeRemoveRequest):
    event = {
        "id": str(uuid.uuid4()),
        "student_id": payload.student_id,
        "action": "remove_badge",
...
async def _compute_analytics_overview(
    ...
    if len(students) == 1:
        ...
        selected_student = {
            ...
            **_focus_component_snapshot(student),
        }
    return {
        "total_students": len(students),
        "classes_count": len(classes),
...
def overview_to_pdf_report(overview: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "quarter1": q1,
        "quarter2": q2,
        "semester": overview.get("semester", 1),
        "quarter": q,
        ...
        "selected_student": overview.get("selected_student"),
    }
