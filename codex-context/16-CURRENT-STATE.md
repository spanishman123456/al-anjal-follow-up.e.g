...
- Student reward feedback is now active on both weekly-entry surfaces: eligible badge awards trigger a five-second full-screen local celebration with dense confetti, falling paper, ribbons, stars and repeated party-popper bursts, glow the new premium badge beside the student name, and no longer depend on the external canvas-confetti CDN. Both International and Arabic row menus now award or remove the badge directly according to its current state, without the International-only reward modal. The PDF no longer auto-opens over the animation; an optional Open certificate toast appears after the visual celebration. Arabic badge certificates and the Arabic certificate dialog are fully Arabic/RTL with Amiri PDF embedding.
