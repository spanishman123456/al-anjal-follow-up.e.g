...
- Awarding an eligible badge uses the shared local `RewardCelebration` overlay in both sections: dense multicolor confetti, ribbons, stars, party-popper cannons, a bilingual student-name banner, badge glow, reduced-motion handling, and no CDN dependency. Arabic Student Management shows the same premium badge beside the student's name instead of plain reward text.
