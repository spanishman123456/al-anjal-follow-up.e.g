...
Entry mode also exposes a bilingual Edit test details action in both sections. It can correct the saved test title and each snapshot class display name under the same optimistic revision guard. Class IDs, roster membership, marks, maximum and academic scope remain immutable; changing a display name updates the existing roster label so lists, analytics, remedial sources and exports stay consistent without renaming the canonical enrollment class.
...
Weekly-entry performance hardening (2026-09-01) does not change any formula: International Student Management requests a lightweight selected-week roster and silently applies a successful one-cell autosave locally instead of recalculating/reloading the entire quarter and broadcasting a global refresh after every blur. Arabic Student Management cancels stale class requests, caches recent class/week responses, and calculates the `/40` quarter average with a MongoDB aggregation instead of transferring all historical weekly documents to Python. Blank and recorded-zero semantics remain unchanged.

Every score-entry surface exposes two deliberately scoped clear actions: exact selected-class clearing and one-student clearing for the current week/assessment/record. No score page exposes a section-wide or all-classes clear control. Weekly follow-up, Q1/Q2 quiz/chapter, Q1/Q2 final, Total Marks, baseline/diagnostic, Arabic weekly `/40`, and Arabic exam `/60` pages all follow this rule. Explicit `null` preserves the distinction from a recorded zero. Arabic class exam clearing deletes only the selected class/year/semester/quarter and leaves `arabic_weekly_scores` history intact; Arabic weekly/student clearing affects only the selected week.

Column-fill tools also require one exact class. Leaving the fill input blank on International matrices means “use this column's maximum”; Arabic weekly and Arabic exam editors provide an explicit maximum-fill selector. Values are staged for review and saved through the normal bulk-save flow.

Arabic Analytics mirrors the International class-first then optional-student workflow while retaining the Arabic `/100` formula. It shows exam completion, performance distribution, class averages, weighted `/40 + /30 + /30` components, every raw exam normalized to its stage maximum, component contribution, student totals, class completion, top/support lists, and the detailed raw/weighted table.

Arabic Reports now renders that same interactive analytical board rather than a separate summary-only page. Its PDF export reproduces the same decision-support structure with seven chart families, an analytical explanation table, class breakdown, detailed raw/weighted student rows, top totals, and completion follow-up. Both PDF and Excel can be scoped to the selected class or one selected student; the backend validates that the student belongs to the permitted class/year/section scope before exporting.
...
## Assessment-driven remedial reports (2026-09-01)

Remedial eligibility is separate from the normal International `46/43` and Arabic proportional performance bands. For this report only, a student is included when an actual recorded score is **strictly below 50%** of the selected maximum. `null`/blank is excluded, recorded zero is included, and exactly 50% is excluded. Sources are the section's baseline diagnostic/pre-test, quarter total, quarter final-test block, or two-quarter semester aggregate. International maxima remain `/50`, `/20`, `/100`; Arabic maxima remain `/100`, `/60`, `/200`. Teacher class assignments and the selected section/year/term are authoritative.
