...
| Pre-test / diagnostic marks | `BaselineAssessments.jsx` entry mode | Variable maximum; independent >=75 / >=50 bands; revision-safe totals; class-scoped maximum fill and confirmed clear; previewed Excel import | `baseline_assessments` | `/baseline-assessments`, `/{id}/scores`, `/{id}/import` | `baseline_assessments.py`, `score_sheet.py` |
...
| Arabic quarter grades | `ArabicGrades.jsx` | `/40 + best theory /30 + practical /30`; Excel import targets one theory attempt only; exact-class smart maximum fill stages one column before normal save; exact-class/exact-term persisted grade deletion; persistent localized load failure + retry | `arabic_quarter_scores` | `/arabic/grades` GET/DELETE, `/arabic/grades/bulk`, `/score-sheet/import`, Arabic exports | page + grading helper/tests + shared import control + server |
