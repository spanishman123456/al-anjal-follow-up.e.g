...
- Maximum, term, class IDs and roster membership cannot be edited through the scores endpoint. A different test/maximum or student scope requires a separate record. The revision-guarded metadata endpoint may correct only the record title and the class display names stored in that snapshot; it mirrors each corrected class label into the existing roster rows without renaming the canonical class, moving students or changing scores. Later enrollment/transfer/promotion/deletion does not silently rewrite historical rosters or clear saved results. Historical retention/deletion must be handled explicitly if a future policy requires it.
