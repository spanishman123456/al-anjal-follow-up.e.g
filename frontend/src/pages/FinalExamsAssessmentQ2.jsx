...
// Assessment part (2nd quarter) = avg weeks 10-18 + best quiz + chapter test, max 30.
function computeAssessmentPartTotal(baseStudent, currentStudent = baseStudent) {
  const avg1018 = baseStudent?.avg_weeks_10_18;
  const studentsTotal =
...
function computeFinalPerformanceLevel(baseStudent, currentStudent = baseStudent) {
  ...
  if (total >= 46) return "on_level";
  if (total >= 43) return "approach";
  return "below";
}

function computeQuarterExamTotal(currentStudent) {
  const practical = Number(currentStudent?.quarter2_practical) ?? 0;
  const theory = Number(currentStudent?.quarter2_theory) ?? 0;
...
export default function FinalExamsAssessmentQ2() {
  ...
  const [fillField, setFillField] = useState("quarter2_practical");
  ...
  const FILL_FIELDS = [
    { key: "quarter2_practical", label: t("quarter2_practical_exam"), max: 10 },
    { key: "quarter2_theory", label: t("quarter2_theoretical_exam"), max: 10 },
  ];
  ...
  const handleClearScores = async () => {
    ...
    const updates = studentsToClear.map((student) => ({
      id: student.id,
      quarter2_practical: null,
      quarter2_theory: null,
    }));
    if (!updates.length) {
  ...
  return (
    <div className="space-y-8" data-testid="final-exams-q2-assessment-page">
      <PageHeader
        pageKey="final_exams_q2"
            ...
            {bulkEditMode ? (
              ...
              <>
                ...
                <Select value={fillField} onValueChange={setFillField}>
                  <SelectTrigger className="w-full sm:w-[220px]" data-testid="final-exams-q2-fill-field">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button type="button" variant="secondary" onClick={handleFillMaxSelectedClass} disabled={filterClass === "all"} data-testid="final-exams-q2-fill-max">
                  <Sparkles className="me-2 h-4 w-4" />
                  {t("fill_max_selected_class")}
                </Button>
                <Button variant="outline" onClick={() => setClearScoresOpen(true)} disabled={filterClass === "all"} data-testid="final-exams-q2-clear-scores">{t("clear_selected_class_scores")}</Button>
              </>
            )}
          </div>
        }
      ...
      <Card data-testid="final-exams-q2-bulk-import-card">
        <CardContent className="flex flex-wrap items-center justify-end gap-3 pt-6">
          <Button variant="secondary" onClick={handleDownloadTemplate} data-testid="final-exams-q2-download-template">
            {t("download_template")}
          </Button>
          <Button variant="secondary" onClick={handleDownloadMarks} data-testid="final-exams-q2-download-marks">
            {t("download_marks")}
          </Button>
          <input
          ...
          />
          <Button onClick={() => bulkFileInputRef.current?.click()} data-testid="final-exams-q2-import-excel">
            {t("import_excel")}
          </Button>
        </CardContent>
      </Card>

      <Card data-testid="final-exams-q2-filter-card">
                ...
                filteredStudents.map((student) => {
                  ...
                  return (
                    <TableRow key={student.id} data-testid={`final-exams-row-${student.id}`}>
                      ...
                      <TableCell className="text-center">
                        <StudentScoreClearButton t={t} studentName={student.full_name} onClear={() => handleClearStudentScores(student)} testId={`final-exams-q2-clear-student-${student.id}`} />
                      </TableCell>
                    </TableRow>
                  );
                })
      ...
      <Dialog open={bulkConfirmOpen} onOpenChange={setBulkConfirmOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleBulkSave} data-testid="final-exams-q2-bulk-confirm">{t("save_changes")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={clearScoresOpen} onOpenChange={setClearScoresOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleClearScores}>{t("clear_selected_class_scores")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
