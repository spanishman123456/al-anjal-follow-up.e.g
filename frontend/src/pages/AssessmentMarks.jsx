...
// Weekly subtotal for this page: avg of first 9 weeks when present, else sum of weekly fields (max 15).
function computeWeeklySubtotalQ1(baseStudent) {
  const avgFirst9 = baseStudent?.avg_first_9_weeks;
  if (avgFirst9 != null && !Number.isNaN(Number(avgFirst9))) {
    return Math.min(STUDENTS_TOTAL_MAX, Math.round(Number(avgFirst9) * 100) / 100);
  }
  return computeStudentsTotal(baseStudent);
}

// Combined total = weekly part (max 15) + best(Quiz 1, Quiz 2) + Chapter Test (max 15), total max 30.
// Use baseStudent for averages / weekly fields and currentStudent for quiz columns (bulk edit preview).
const COMBINED_TOTAL_MAX = 30;
function computeCombinedTotal(baseStudent, currentStudent = baseStudent) {
  const studentsTotal = computeWeeklySubtotalQ1(baseStudent);
...
function computeAssessmentPerformanceLevel(baseStudent, currentStudent = baseStudent) {
  ...
  const hasAvgFirst9 =
    baseStudent?.avg_first_9_weeks != null && !Number.isNaN(Number(baseStudent.avg_first_9_weeks));
  const hasStudents =
    hasAvgFirst9 ||
...
export default function AssessmentMarks() {
  ...
  const [fillField, setFillField] = useState("quiz1");
  ...
  const handleChapterTest1Blur = async (student) => {
    const current = { ...student, ...bulkScores[student.id] };
    await saveScoreOnBlur(student, "chapter_test1_practical", parseScore(current.chapter_test1_practical));
  };

  const handleQuiz1Blur = async (student) => {
    const current = { ...student, ...bulkScores[student.id] };
    await saveScoreOnBlur(student, "quiz1", parseScore(current.quiz1));
  };

  const handleQuiz2Blur = async (student) => {
    const current = { ...student, ...bulkScores[student.id] };
    await saveScoreOnBlur(student, "quiz2", parseScore(current.quiz2));
  };

  const FILL_FIELDS = [
    { key: "quiz1", label: t("quiz1"), max: 5 },
    { key: "quiz2", label: t("quiz2"), max: 5 },
    { key: "chapter_test1_practical", label: t("assessment_chapter_test"), max: 10 },
  ];
  ...
  const handleClearScores = async () => {
    ...
    // Clear only assessment fields for the selected class for this week.
  ...
  return (
    <div className="space-y-8" data-testid="assessment-marks-page">
        ...
        action={
          <div className="flex flex-wrap gap-2">
            ...
            {bulkEditMode ? (
              ...
              <>
                <Button variant="outline" onClick={startBulkEdit} data-testid="assessment-edit-scores">
                  {t("edit_scores")}
                </Button>
                <Select value={fillField} onValueChange={setFillField}>
                  <SelectTrigger className="w-full sm:w-[220px]" data-testid="assessment-fill-field">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button type="button" variant="secondary" onClick={handleFillMaxSelectedClass} disabled={filterClass === "all"} data-testid="assessment-fill-max">
                  <Sparkles className="me-2 h-4 w-4" />
                  {t("fill_max_selected_class")}
                </Button>
                <Button variant="outline" onClick={() => setClearScoresOpen(true)} disabled={filterClass === "all"} data-testid="assessment-clear-scores">
      ...
      <Card data-testid="assessment-bulk-import-card">
        <CardContent className="space-y-3 pt-6">
          ...
          <ScoreSheetImportControl
            ...
            targets={[
              { value: "quiz1", label: `${t("quiz1")} (5)` },
              { value: "quiz2", label: `${t("quiz2")} (5)` },
            ]}
            ...
            testIdPrefix="assessment-score-import"
          />
          <Button
            variant="secondary"
            onClick={handleDownloadMarks}
            data-testid="assessment-download-marks"
          >
      ...
      <Card data-testid="assessment-filter-card">
        <CardContent className="grid gap-4 pt-6 md:grid-cols-2 xl:grid-cols-6">
          ...
          >
            <SelectTrigger data-testid="assessment-class-filter-trigger">
          ...
          <Select value={performanceFilter} onValueChange={setPerformanceFilter} data-testid="assessment-performance-filter">
            <SelectTrigger>
          ...
          <Button variant="outline" onClick={resetFilters} data-testid="assessment-reset-filters">
            {t("reset_filters")}
          </Button>
        </CardContent>
      </Card>

      <Card noHoverPop data-testid="assessment-table-card">
        <CardContent className="pt-6">
          <Table data-testid="assessment-marks-table">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center" title={t("total_score_tooltip_q1")}>
                  {t("total_score")}
                </TableHead>
                    ...
                    <TableRow key={student.id} data-testid={`assessment-row-${student.id}`}>
                      ...
                      <TableCell className="text-center">
                        <Input
                          ...
                          onBlur={() => handleQuiz1Blur(student)}
                          aria-label={t("quiz1")}
                      ...
                      <TableCell className="text-center">
                        <Input
                          ...
                          onBlur={() => handleQuiz2Blur(student)}
                          aria-label={t("quiz2")}
                      ...
                      <TableCell className="text-center">
                        <Input
                          ...
                          onBlur={() => handleChapterTest1Blur(student)}
                          aria-label={t("chapter_test1_practical")}
                      ...
                      <TableCell
                        ...
                        title={t("total_score_tooltip_q1")}
                      >
                      ...
                      <TableCell className="text-center">
                        <StudentScoreClearButton
                          ...
                          testId={`assessment-clear-student-${student.id}`}
                        />
                      </TableCell>
                    </TableRow>
                  );
      ...
      <Dialog open={bulkConfirmOpen} onOpenChange={setBulkConfirmOpen}>
        <DialogContent data-testid="assessment-bulk-confirm-dialog">
          <DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button variant="success" onClick={handleBulkSave} data-testid="assessment-bulk-confirm">
              {t("save_changes")}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={clearScoresOpen} onOpenChange={setClearScoresOpen}>
        <DialogContent data-testid="assessment-clear-dialog">
          <DialogHeader>
